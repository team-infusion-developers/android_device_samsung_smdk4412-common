...
status_t GraphicBufferMapper::lockAsync(buffer_handle_t handle,
        uint64_t inConsumerUsage, uint64_t inProducerUsage, const Rect& bounds, void** vaddr, int fenceFd)
{
    return lockAsync(handle, inConsumerUsage, inProducerUsage, bounds, vaddr, fenceFd);
}*/
...
status_t GraphicBufferMapper::lockAsync(buffer_handle_t handle,
        uint64_t inConsumerUsage, uint64_t inProducerUsage __unused, const Rect& bounds,
        void** vaddr, int fenceFd)
{
    ...
    if (mAllocMod->common.module_api_version >= GRALLOC_MODULE_API_VERSION_0_3) {
        err = mAllocMod->lockAsync(mAllocMod, handle, static_cast<int>(inConsumerUsage),
                bounds.left, bounds.top, bounds.width(), bounds.height(),
                vaddr, fenceFd);
    } else {
        if (fenceFd >= 0) {
        ...
        err = mAllocMod->lock(mAllocMod, handle, static_cast<int>(inConsumerUsage),
                bounds.left, bounds.top, bounds.width(), bounds.height(),
                vaddr);
    }
